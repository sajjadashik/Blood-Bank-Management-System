...
function getallrecieverinfo()
{
$query = "select * from reciever_info";
$result = get($query);
return $result;
}
function retrive($id){
		$query = "select * from reciever_info where id=$id";
		$result = get($query);
		if(count($result) > 0){
			return $result[0];
		}
...
{
	$query="update reciever_info set Name='$name',Email='$email',Phone_Number='$phonenumber',date='$day',month='$month',year='$year',Gender='$gender',Street='$street',City='$city',State='$state',Zip='$zip' where id=$id";
	execute($query);
	header("Location: admin_recieverInfo.php");
}
...
function checkuname($username){
		$query = "select * from reciever_info where username='$username'";
		$result=get($query);
		if(count($result) > 0){
			return "false";
		}
